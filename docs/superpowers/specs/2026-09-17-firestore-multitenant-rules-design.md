...
All tests run against the real Firestore Emulator (not mocked), via `@firebase/rules-unit-testing`'s `initializeTestEnvironment`, driven by `firebase emulators:exec --project demo-nextfood --only firestore "vitest run"`. Only the Firestore emulator is needed — `@firebase/rules-unit-testing` mocks authentication state itself (`authenticatedContext()`/`unauthenticatedContext()`) and does not require a running Auth emulator. The `demo-` prefix is a documented Firebase safeguard: demo projects only ever talk to local emulators, never to a real project, so there is no code path by which running this suite could touch `nextfood-76bc3` or incur billing. Fixture data (e.g. "Tenant B already has an order") is seeded with `testEnv.withSecurityRulesDisabled(...)`, so fixture setup never depends on the rules being tested. Each test clears Firestore state (`testEnv.clearFirestore()`) between cases for isolation.
